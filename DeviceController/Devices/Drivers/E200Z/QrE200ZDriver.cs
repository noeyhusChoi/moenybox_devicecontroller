...
public sealed class QrE200ZDriver : DeviceBase
{
    ...
    public QrE200ZDriver(DeviceDescriptor descriptor, ITransport transport)
    ...
    public override async Task<StatusSnapshot> GetStatusAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (!result.Success)
                alarms.Add(CreateAlarm(new ErrorCode("DEV", "QR", "STATUS", "ERROR"), string.Empty, Severity.Warning));
        }
        catch
        {
            alarms.Add(CreateAlarm(new ErrorCode("DEV", "QR", "STATUS", "ERROR"), string.Empty, Severity.Warning));
