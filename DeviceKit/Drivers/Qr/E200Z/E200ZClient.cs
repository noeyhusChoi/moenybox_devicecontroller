...
namespace DeviceKit.Drivers.E200Z;
...
internal sealed class E200ZClient : IAsyncDisposable
{
    ...
    public E200ZClient(DeviceDescriptor descriptor)
    {
        ArgumentNullException.ThrowIfNull(descriptor);
        _channel = new TransportChannel(StreamPortFactory.Create(descriptor), new E200ZFramer());
    }
    ...
    public Task<DeviceCommandResponse> SetHostTriggerModeAsync(bool saveToFlash, CancellationToken ct)
    {
    ...
    public Task<DeviceCommandResponse> SetAutoInductionTriggerModeAsync(bool saveToFlash, CancellationToken ct)
    {
        var pkt = SsiPacket.CreateParamByte(0x8A, 0x09, saveToFlash);
        return SendPacketAsync(pkt, SsiOpcode.CMD_ACK, E200ZTimeouts.DefaultCommandMs, ct);
    }

    public Task<DeviceCommandResponse> SetDecodeDataPacketFormatAsync(byte value, bool saveToFlash, CancellationToken ct)
    {
        var pkt = SsiPacket.CreateParamByte(0xEE, value, saveToFlash);
        return SendPacketAsync(pkt, SsiOpcode.CMD_ACK, E200ZTimeouts.DefaultCommandMs, ct);
    }

    private async Task<DeviceCommandResponse> SendPacketAsync(SsiPacket packet, SsiOpcode expected, int timeoutMs, CancellationToken ct)
    {
        ...
        return new DeviceCommandResponse(true, Data: parsedResp.Data);
    }
