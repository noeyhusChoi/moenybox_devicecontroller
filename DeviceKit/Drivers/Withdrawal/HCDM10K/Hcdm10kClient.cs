...
namespace DeviceKit.Drivers.HCDM;
...
internal sealed class Hcdm10kClient : IAsyncDisposable
{
    ...
    public Hcdm10kClient(DeviceDescriptor descriptor, ILogger? logger = null)
    {
        ArgumentNullException.ThrowIfNull(descriptor);
        _logger = logger ?? NullLogger.Instance;
        _channel = new TransportChannel(StreamPortFactory.Create(descriptor), new Hcdm10kFramer());
    }
    ...
    public async Task<DeviceCommandResponse> SendCommandAsync(Hcdm10kCommand command, byte[]? data = null, int processTimeoutMs = 5000, CancellationToken ct = default)
    {
        if (!_started)
        ...
        _logger.LogDebug("[HCDM10K] TX {Command}: {Frame}", command, BitConverter.ToString(frame));
        ...
        while (true)
        {
            ...
            _logger.LogDebug("[HCDM10K] RX: {Frame}", BitConverter.ToString(full));
