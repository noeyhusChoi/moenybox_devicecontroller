...
/// </summary>
public sealed class DeviceRuntimePort : IDeviceRuntimePort, IDeviceRuntimeStatusEvents, IAsyncDisposable
{
    ...
    private static int CalculateSchedulerTickMs(IEnumerable<DeviceDescriptor> descriptors, DeviceRuntimeOptions options)
    {
    ...
    private static int GetPollingMs(DeviceDescriptor descriptor, DeviceRuntimeOptions options)
    ...
    private static async Task SafeDisposeAsync(IDeviceDriver? device, ITransport? transport, CancellationToken cancellationToken)
    {
        try
        {
