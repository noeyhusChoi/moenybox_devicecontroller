...
public abstract class DeviceBase : IDevice, IAsyncDisposable
{
    ...
    protected StatusSnapshot CreateSnapshot(IEnumerable<StatusEvent>? alerts = null, AlertSource alertScope = AlertSource.Status)
        => new()
        {
            Name = Name,
            Model = Model,
    ...
    protected StatusEvent CreateAlert(string code, string message, Severity severity = Severity.Error, AlertSource source = AlertSource.Status)
        => new(code, message, severity, DateTimeOffset.UtcNow, Source: source);
