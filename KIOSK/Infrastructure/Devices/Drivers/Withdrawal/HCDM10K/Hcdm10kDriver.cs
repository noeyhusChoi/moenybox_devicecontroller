...
/// </summary>
public sealed class Hcdm10kDriver : DeviceDriverBase, IWithdrawalDriver
{
    ...
    private void ParseSensors(byte[] x, List<StatusEvent> alerts)
    {
        ...
        if ((shutter & Hcdm10kShutterBits.ShutOpen) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "SHUT_OPEN"), string.Empty, Severity.Info));
        if ((shutter & Hcdm10kShutterBits.ShutClose) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "SHUT_CLOSE"), string.Empty, Severity.Info));
        if ((shutter & Hcdm10kShutterBits.ShutIn1) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "SHUT_IN1"), string.Empty, Severity.Info));
        if ((shutter & Hcdm10kShutterBits.ShutIn2) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "SHUT_IN2"), string.Empty, Severity.Info));
        if ((shutter & Hcdm10kShutterBits.ShutIn3) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "SHUT_IN3"), string.Empty, Severity.Info));

        var status = x[(int)Hcdm10kSensorIndex.Status];
        if ((status & Hcdm10kStatusBits.Msol) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "MSOL_COLLECT"), string.Empty, Severity.Info));
        else
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "MSOL_DISPENSE"), string.Empty, Severity.Info));
        if ((status & Hcdm10kStatusBits.CisOpen) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "CIS_OPEN"), string.Empty, Severity.Warning));
        if ((status & Hcdm10kStatusBits.RejectBoxOpen) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "REJECT_BOX_UNLOCK"), string.Empty, Severity.Warning));

        var gate = (Hcdm10kGateFlags)x[(int)Hcdm10kSensorIndex.Gate];
        if (gate.HasFlag(Hcdm10kGateFlags.Exit1Detected))
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "EXIT1_DETECTED"), string.Empty, Severity.Info));
        if (gate.HasFlag(Hcdm10kGateFlags.RejectInDetected))
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "REJECT_IN_DETECTED"), string.Empty, Severity.Info));
        if (gate.HasFlag(Hcdm10kGateFlags.Gate1Detected))
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "GATE1_DETECTED"), string.Empty, Severity.Info));
        if (gate.HasFlag(Hcdm10kGateFlags.Gate2Detected))
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "GATE2_DETECTED"), string.Empty, Severity.Info));
        if (gate.HasFlag(Hcdm10kGateFlags.ScanStart))
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "SCAN_START"), string.Empty, Severity.Info));
    ...
    private void ParseCassette(byte value, int cassetteNo, List<StatusEvent> alerts)
    {
        ...
        if ((value & Hcdm10kCassetteBits.Skew1) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "CASSETTE_SKEW1"), string.Empty, Severity.Info));
        if ((value & Hcdm10kCassetteBits.Skew2) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "CASSETTE_SKEW2"), string.Empty, Severity.Info));
        if ((value & Hcdm10kCassetteBits.NearEnd) == 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "CASSETTE_NEAR_END"), string.Empty, Severity.Warning));
        if ((value & Hcdm10kCassetteBits.Mount) == 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "CASSETTE_NOT_MOUNTED"), string.Empty, Severity.Warning));
        if ((value & Hcdm10kCassetteBits.Id1A) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "CASSETTE_ID1A"), string.Empty, Severity.Info));
        if ((value & Hcdm10kCassetteBits.Id2A) != 0)
            alerts.Add(CreateAlert(new ErrorCode("DEV", "WITHDRAWAL", "STATUS", "CASSETTE_ID2A"), string.Empty, Severity.Info));
    }

    private IReadOnlyDictionary<string, IDeviceCommandHandler> CreateHandlers(Hcdm10kClient client)
