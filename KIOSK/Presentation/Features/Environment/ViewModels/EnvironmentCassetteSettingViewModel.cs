...
namespace KIOSK.Presentation.Features.Environment.ViewModels
{
    public partial class EnvironmentCassetteSettingViewModel : ObservableObject, INavigable
    {
        ...
        public EnvironmentCassetteSettingViewModel(IPopupService popup, WithdrawalCassetteServiceV2 withdrawalCassetteService)
        {
            ...
            WithdrawalCassette = new ObservableCollection<WithdrawalCassetteModel>();
        }

        public async Task OnLoadAsync(object? parameter, CancellationToken ct)
        {
            WithdrawalCassette.Clear();
            var slots = await _withdrawalCassetteService.GetSlotsAsync();
            foreach (var slot in slots)
                WithdrawalCassette.Add(slot);
        }
        ...
        [RelayCommand]
        private async Task SaveAll(object obj)
        {
