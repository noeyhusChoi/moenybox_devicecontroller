...
public partial class CashExchangeShellViewModel : ObservableObject, IModalSourceViewModel, IProgressChromeShellViewModel
{
    ...
    public CashExchangeShellViewModel(
    ...
    private void ConfigureShellActions(ExchangeStep step)
    {
        if (step == ExchangeStep.CurrencySelection && CurrentStepViewModel is not null)
            CurrentStepViewModel.SecondaryCommand = RequestEntryBackCommand;
    }

    [RelayCommand]
    private Task RequestEntryBackAsync()
    {
